 ...
 * render the scene multiple times, adding shadows for one light at a time
 */
public class ShadowMapShader extends BaseShader
{
    ...
    public ShadowMapShader(final Renderable renderable, final ShaderProgram shaderProgramModelBorder, final ShadowSystem sys)
    {
	system = sys;
	...
	for (final AbstractShadowLight light : system.getLights())
	{
	    ...
	    {
		// We could use the classic depth test (less or equal), but
		// strict equality works fine on next passes as depth buffer
		// already contains our scene
